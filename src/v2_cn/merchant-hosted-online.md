...
[在线测试API](https://pay.latipay.net/api-console-merchant-hosted-online)
...
| 参数  | 结果 |
| -------------| ------| ------
| host\_type = "1" | 使用该参数，支付接口会返回一张二维码图片（Base64格式） |
...
```
<p class="tip">支付宝直接跳转到支付宝收银台，银行卡支付会跳转到一个中间页面供用户输入银行卡信息，微信支付会返回如下内容：</p>
